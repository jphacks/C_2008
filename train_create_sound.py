...
def main(efficientModel: int):
    '''
    labels = os.listdir('image_dataset')
    labels.remove('なし')
    '''
    ...
    filename = []
    label = []
    for label_name in os.listdir('image_dataset'):
        for img_filename in os.listdir(os.path.join('image_dataset', label_name)):   
            filename.append(os.path.join(label_name, img_filename))
            tmp = [0] * len(labels)
            if label_name in labels:
                tmp[labels.index(label_name)] = 1
            label.append(tmp)

    file_data = pd.DataFrame({'filename': filename})
    label_data = pd.DataFrame(np.array(label, dtype=int), columns=labels)
    data = pd.concat([file_data, label_data], axis=1)
    '''
